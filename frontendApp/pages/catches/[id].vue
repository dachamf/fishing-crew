...
import type {FishingCatch, PhotoLite, GroupMember} from '~/types/api'
...
async function askReject() {
  // brzi confirm bez napomene:
  const ok = await confirmModal({
    title: 'Potvrdi odbijanje',
    message: 'Ova radnja će označiti ulov kao odbijen.',
    confirmText: 'Odbij',
    cancelText: 'Odustani',
    tone: 'danger'
  })
...
const myPending = computed(() =>
  (data.value?.confirmations || []).some(c => c.confirmed_by === myId.value && c.status === 'pending')
);
...
const nameById = computed<Record<number, string>>(() =>
  Object.fromEntries((members.value || []).map(m => [m.id, m.display_name || m.name || `#${m.id}`]))
)
...
async function requestConfirmations() {
  ...
  if (!ids.length) return alert('Izaberi bar jednog člana.')
  try {
        ...
        <div class="card-body space-y-3">
          ...
          <div v-if="canNominate" class="form-control">
            ...
            <div class="dropdown">
              <div tabindex="0" class="btn">
                {{ selectedMemberIds.length ? `Izabrano: ${selectedMemberIds.length}` : 'Izaberi članove' }}
              </div>
              <ul
                tabindex="0"
                class="dropdown-content menu p-2 shadow bg-base-100 rounded-box w-72 max-h-64 overflow-auto"
              >
                <li v-if="membersLoading" class="opacity-70 px-2 py-1">Učitavanje…</li>
                <li v-else-if="!(members?.length)" class="opacity-70 px-2 py-1">Nema članova</li>
            ...
            <div v-if="selectedMemberIds.length" class="mt-2 flex flex-wrap gap-2">
              <span v-for="uid in selectedMemberIds" :key="uid" class="badge badge-outline">
                {{ nameById[uid] || ('#' + uid) }}
              </span>
            </div>
          ...
          <div class="form-control">
            ...
            <div v-if="myPending" class="mt-2 join gap-1">
              ...
              <button class="btn btn-error join-item" @click="openReject">Odbij</button>
            </div>
            ...
            >
              ...
              <textarea v-model="note" rows="3" class="textarea textarea-bordered w-full" placeholder="Zašto odbijaš?"/>
            </UiConfirmDialog>
          </div>
