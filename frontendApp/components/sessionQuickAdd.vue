...
async function createSession() {
  if (!props.groupId)
    return toast.info("Izaberi grupu pre kreiranja sesije");
  creating.value = true;
  try {
    ...
    console.error(e);
    toast.error(toErrorMessage(e?.response?.data?.message) || "Greška pri kreiranju sesije");
  }
  finally {
...
<template>
  <div class="space-y-2">
    ...
    <div v-if="!newMode" class="flex items-center gap-2">
      ...
      <button class="btn btn-outline btn-sm" @click="newMode = true">
        Nova
      </button>
    </div>
    ...
    <div v-else class="rounded-xl border border-base-300 p-3 bg-base-200 space-y-2">
      ...
      <div class="flex items-center gap-2 justify-end">
        <button class="btn btn-ghost btn-sm" @click="newMode = false">
          Otkaži
        </button>
        <button
