...
function validate() {
  errors.value = {}
  if (!form.title.trim()) errors.value.title = 'Obavezan naziv.'
  if (!form.start_at_local) errors.value.start_at_local = 'Obavezan datum i vreme.'
  ...
  <form class="space-y-4" @submit.prevent="submit" :aria-busy="busy">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
      <div>
        ...
        <input v-model="form.location_name" class="input input-bordered w-full" placeholder="Srebrno jezero" />
      </div>
      <div>
        <label class="label"><span class="label-text">Datum & vreme</span></label>
        <input v-model="form.start_at_local" type="datetime-local" class="input input-bordered w-full" required />
        <p v-if="errors.start_at_local" class="text-error text-sm mt-1">{{ errors.start_at_local }}</p>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
      <div>
        ...
        <input v-model.number="form.latitude" type="number" step="0.000001" min="-90" max="90" class="input input-bordered w-full" />
        <p v-if="errors.latitude" class="text-error text-sm mt-1">{{ errors.latitude }}</p>
      </div>
      <div>
        <label class="label"><span class="label-text">Longitude</span></label>
        <input v-model.number="form.longitude" type="number" step="0.000001" min="-180" max="180" class="input input-bordered w-full" />
        <p v-if="errors.longitude" class="text-error text-sm mt-1">{{ errors.longitude }}</p>
      </div>
    </div>
