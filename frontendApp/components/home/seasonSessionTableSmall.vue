...
type Row = FishingSession & { catches_count?: number };
...
/** ── Grupisanje po mesecu (sekundarni header) ───────────────── */
...
function monthLabelFromKey(key: string) {
  ...
  // “sr-RS” mesec + godina; capitalize prvi znak
  const raw = dt.toLocaleDateString("sr-RS", { month: "long", year: "numeric" });
...
const groups = computed(() => {
  ...
  for (const s of items.value) {
    const k = monthKey(s.started_at || (s as any)?.created_at || null);
    if (!buckets.has(k))
      buckets.set(k, []);
    buckets.get(k)!.push(s);
  }
  // sort po ključu desc; “no-date” ide na kraj
      ...
      <div v-else>
        ...
        <div v-else class="overflow-x-auto">
          <table class="table">
            ...
            <!-- Sekundarni header po mesecu -->
            <tbody v-for="g in groups" :key="g.key">
              <tr class="bg-base-200/60">
                <td :colspan="6" class="py-2 text-sm font-semibold">
                  {{ g.label }}
                </td>
              </tr>

              <tr v-for="s in g.rows" :key="s.id">
